...
display_video () {
	sshpass -p agapeone ssh "pi@$2.local" ffmpeg -an -f video4linux2 -s "$3" -c:v mjpeg -i "/dev/video$1" -c:v libx264 -preset ultrafast -tune zerolatency -f matroska - | mpv --autofit=100%x100% --fs --title="video$1" --profile=low-latency --demuxer=mkv -
	#sshpass -p agapeone ssh "pi@$2.local" ffmpeg -an -f video4linux2 -s "$3" -c:v mjpeg -i "/dev/video$1" -c:v copy -f matroska - | ffplay -
}

for i in $(seq "${1:-3}"); do
	sleep 1 && display_video $((0+(i-1)*2)) "${2:-raspberrypi}" "${3:-1920x1080}" &
done
